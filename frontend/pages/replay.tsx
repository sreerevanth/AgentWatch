...
import { api, AgentEvent, ExecutionStatus } from '../lib/api'
...
export default function ReplayStudio() {
  ...
  const [activeTab, setActiveTab] = useState<'details' | 'simulation' | 'diff' | 'analysis'>('details')
  const [simResult, setSimResult] = useState<SimulationResult | null>(null)
  const [isSimulating, setIsSimulating] = useState(false)
  const [replacementValue, setReplacementValue] = useState('')

  const url = sessionParam ? `${API_BASE}/sessions/${sessionParam}/replay` : null
  const { data, isLoading, error } = useSWR<ReplaySession>(url, fetcher)
  ...
  const handleSimulate = async () => {
    ...
    try {
      const result = await api.simulate(sessionParam, {
        rewind_to_step: step,
  ...
  if (!sessionParam) return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-[#0b1020] text-slate-300">
      ...
      <p className="text-slate-500 mb-8">Enter a session ID to begin analysis</p>
      <div className="flex gap-2">
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === 'Enter') router.push(`/replay?session=${e.currentTarget.value}`)
          }}
        />
      </div>
    </div>
      ...
      <header className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-[#0f172a]/50">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-slate-500 font-mono">{sessionParam}</div>
          </div>
        </div>

        <div className="flex items-center gap-4 bg-slate-900/50 rounded-full px-4 py-1.5 border border-slate-800">
          <button onClick={() => setStep(0)} className="hover:text-pink-400 transition-colors"><SkipBack size={18} /></button>
          <button onClick={() => setStep(s => Math.max(0, s - 1))} className="hover:text-pink-400 transition-colors"><ChevronLeft size={22} /></button>
          ...
          <button onClick={() => setStep(s => Math.min(events.length - 1, s + 1))} className="hover:text-pink-400 transition-colors"><ChevronRight size={22} /></button>
          <button onClick={() => setStep(events.length - 1)} className="hover:text-pink-400 transition-colors"><SkipForward size={18} /></button>
        ...
        <div className="flex items-center gap-3 text-xs font-medium uppercase tracking-wider text-slate-500">
          ...
          <div className={`flex items-center gap-1.5 px-3 py-1.5 bg-slate-900 rounded-md border border-slate-800 ${data?.status === 'success' ? 'text-green-400' : 'text-red-400'}`}>
            <History size={14} />
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-80 border-r border-slate-800 bg-[#0f172a]/30 flex flex-col">
          <div className="p-4 border-b border-slate-800 bg-slate-900/20 flex justify-between items-center">
            <h3 className="text-xs font-bold uppercase tracking-widest text-slate-500">Timeline Events</h3>
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-start gap-3 p-3 rounded-lg text-left transition-all group ${
                    isSelected ? 'bg-pink-600/10 border border-pink-500/30' : 'hover:bg-slate-800/50'
                  }`}
                >
                  <div className="flex flex-col items-center mt-1">
                    <div className={`w-2 h-2 rounded-full mb-1 ${isSelected ? 'bg-pink-500 shadow-[0_0_8px_rgba(236,72,153,0.8)]' : isPast ? 'bg-slate-700' : 'bg-slate-800'}`}></div>
                    <div className="w-[1px] h-8 bg-slate-800"></div>
                  </div>

                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-center mb-0.5">
                      <div className={`text-[10px] font-mono ${isSelected ? 'text-pink-400' : 'text-slate-500'}`}>
                        STEP {s.event.step_number}
                      </div>
                      <div className="text-[10px] text-slate-600 font-mono">
                        {new Date(s.event.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      <div
                        ...
                        style={{ backgroundColor: `${EVENT_COLOR[s.event.event_type] ?? '#475569'}20`, color: EVENT_COLOR[s.event.event_type] }}
                      >
                        <Icon size={12} />
                      </div>
                      <span className={`text-sm font-medium truncate ${isSelected ? 'text-white' : 'text-slate-300'}`}>
                        {s.event.tool_call?.tool_name || s.event.event_type.replace('.', ' ')}
                      </span>
                    </div>
                    ...
                    {s.annotations.length > 0 && (
                      <div className="mt-2 flex flex-wrap gap-1">
                        {s.annotations.map((ann, idx) => (
                          <span key={idx} className="text-[9px] px-1.5 py-0.5 rounded bg-red-500/20 text-red-400 border border-red-500/20">
                            {ann}
                          </span>
                        ))}
                      </div>
        ...
        <main className="flex-1 flex flex-col bg-[#0b1020] overflow-hidden">
            ...
            <div className="h-full flex flex-col">
              ...
              <div className="flex-1 font-mono text-sm text-purple-200/90 leading-relaxed overflow-y-auto custom-scrollbar pr-4">
                {currentEvent?.planner_output_preview ? (
                  <Typewriter text={currentEvent.planner_output_preview} speed={20} />
                ) : (
          ...
          <nav className="flex items-center px-6 border-b border-slate-800 bg-[#0f172a]/20">
            {[
              ...
              { id: 'analysis', label: 'Failure Analysis', icon: AlertTriangle },
            ].map(tab => (
              <button
                key={tab.id}
                ...
                {currentEvent ? (
                  <>
                    <div className="flex items-start justify-between">
                      <div>
                        <div className="text-[10px] font-mono text-slate-500 uppercase mb-1">Current Event</div>
                        <h2 className="text-2xl font-bold text-white flex items-center gap-3">
                          {currentEvent.tool_call?.tool_name || currentEvent.event_type}
                          <span className={`text-xs px-2 py-0.5 rounded-full font-mono ${
                            currentEvent.status === 'success' ? 'bg-green-500/20 text-green-400' :
                            currentEvent.status === 'failure' ? 'bg-red-500/20 text-red-400' :
                      ...
                      <div className="text-right">
                        <div className="text-[10px] font-mono text-slate-500 uppercase mb-1">Timestamp</div>
                        <div className="text-sm font-mono text-slate-300">{new Date(currentEvent.timestamp).toISOString()}</div>
                      </div>
                    </div>
                    ...
                    {currentEvent.tool_call && (
                      <div className="bg-slate-900/80 rounded-xl border border-slate-800 overflow-hidden shadow-2xl">
                        <div className="px-4 py-2 bg-slate-800/50 border-b border-slate-800 flex justify-between items-center">
                          ...
                          <span className="text-[10px] font-mono text-slate-500">{currentEvent.tool_call.tool_name}</span>
                        </div>
                        <div className="p-4 space-y-4">
                          {currentEvent.tool_call.raw_command && (
                            <div>
                              <div className="text-[10px] text-slate-500 uppercase mb-1.5 font-bold">Command</div>
                          ...
                          <div>
                            <div className="text-[10px] text-slate-500 uppercase mb-1.5 font-bold">Arguments</div>
                            <pre className="p-3 bg-black/50 rounded-lg text-slate-400 text-xs font-mono overflow-x-auto">
                              {JSON.stringify(currentEvent.tool_call.arguments, null, 2)}
                            </pre>
                          </div>
                        </div>
                      </div>
                    ...
                    {currentEvent.tool_result && (
                      <div className="bg-slate-900/80 rounded-xl border border-slate-800 overflow-hidden shadow-2xl">
                        <div className="px-4 py-2 bg-slate-800/50 border-b border-slate-800 flex justify-between items-center">
                          ...
                          <span className="text-[10px] font-mono text-slate-500">{currentEvent.tool_result.tool_name}</span>
                        </div>
                        <div className="p-4">
                          {currentEvent.tool_result.error ? (
                            <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-3">
                              <div className="text-[10px] text-red-400 uppercase font-bold mb-1">Execution Error</div>
                              <pre className="text-red-300 text-xs font-mono whitespace-pre-wrap">{currentEvent.tool_result.error}</pre>
                            </div>
                          ) : (
                            <div>
                              <div className="text-[10px] text-slate-500 uppercase mb-1.5 font-bold">Output</div>
                              <pre className="p-3 bg-black/50 rounded-lg text-green-300 text-xs font-mono overflow-x-auto max-h-[400px]">
                                {typeof currentEvent.tool_result.output === 'string'
                                  ? currentEvent.tool_result.output
                                  : JSON.stringify(currentEvent.tool_result.output, null, 2)}
                              </pre>
                            </div>
                          )}
                        </div>
                    ...
                    {currentEvent.safety && (
                      <div className={`rounded-xl border p-4 flex gap-4 ${
                        currentEvent.safety.blocked ? 'bg-red-500/10 border-red-500/30' : 'bg-orange-500/10 border-orange-500/30'
                      }`}>
                        <div className={`p-3 rounded-full h-fit ${currentEvent.safety.blocked ? 'bg-red-500 text-white' : 'bg-orange-500 text-white'}`}>
                          <ShieldAlert size={24} />
                        </div>
                        <div>
                          <h3 className="text-lg font-bold mb-1">
                            Safety {currentEvent.safety.blocked ? 'Block' : 'Warning'}
                            <span className="ml-3 text-xs opacity-60 font-mono">Risk Level: {currentEvent.safety.risk_level}</span>
                          </h3>
                          <ul className="space-y-1 list-disc list-inside text-sm text-slate-300">
                            {currentEvent.safety.reasons.map((r, i) => <li key={i}>{r}</li>)}
                          </ul>
                          {currentEvent.safety.matched_policies.length > 0 && (
                            <div className="mt-3 flex gap-2">
                              {currentEvent.safety.matched_policies.map(p => (
                                <span key={p} className="text-[10px] px-2 py-0.5 bg-slate-800 rounded text-slate-400 border border-slate-700 font-mono uppercase">
                                  {p}
                                </span>
                              ))}
                            </div>
              ...
              <div className="max-w-3xl space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                      <h3 className="text-lg font-bold">Counterfactual Simulation</h3>
                      <p className="text-xs text-slate-500">Modify this step's outcome and re-simulate the agent's path.</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-6">
                    <div className="flex items-center gap-4 p-4 bg-slate-800/30 rounded-xl border border-slate-800">
                      ...
                      <div className="flex-1">
                        <div className="text-xs font-bold uppercase text-slate-500 mb-1">Rewind Target</div>
                        <div className="font-medium">{currentEvent?.tool_call?.tool_name || currentEvent?.event_type}</div>
                      </div>
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-slate-500 uppercase mb-2">Simulated Replacement Result</label>
                      <textarea
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-4 bg-blue-500/5 border border-blue-500/10 rounded-xl">
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed">
                      Branching allows you to test hypotheses. "What if the tool returned an error here?"
                      or "What if this safety block was bypassed?"
                    </p>
                  </div>
                  <div className="p-4 bg-purple-500/5 border border-purple-500/10 rounded-xl">
                    <h4 className="text-xs font-bold text-purple-400 uppercase mb-2">Next Steps</h4>
                    <p className="text-xs text-slate-400 leading-relaxed">
                      After simulating, use the Trajectory Diff tab to see where the agent's logic diverged
                      from the original recorded session.
                    </p>
                  </div>
                </div>
              </div>
                  ...
                  <div className="space-y-8">
                    ...
                    <div className="grid grid-cols-1 gap-1 relative">
                      ...
                      {simResult.alternate_events.map((ev, idx) => {
                        const isDivergencePoint = idx === simResult.diverged_at_step
                        const isAfterDivergence = simResult.diverged_at_step !== null && idx >= simResult.diverged_at_step
                        const originalEvent = simResult.original_events[idx]
                        const isDifferent = originalEvent && JSON.stringify(originalEvent.tool_call) !== JSON.stringify(ev.tool_call)

                        return (
                          <div key={ev.event_id} className={`flex gap-6 p-4 rounded-xl transition-all ${
                            isDivergencePoint ? 'bg-pink-500/10 border border-pink-500/20' :
                            isAfterDivergence ? 'bg-slate-900/40 opacity-80' : 'hover:bg-slate-900/40'
                          }`}>
                            ...
                            <div className="flex-1 space-y-3">
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                  <span className={`text-xs font-bold uppercase ${isAfterDivergence ? 'text-pink-400' : 'text-slate-400'}`}>
                              ...
                              {isDifferent && (
                                <div className="grid grid-cols-2 gap-4">
                                  <div className="p-3 bg-slate-900/60 rounded-lg border border-slate-800 opacity-60">
                                    <div className="text-[9px] font-bold uppercase text-slate-500 mb-1">Original Payload</div>
                                    <div className="text-[10px] font-mono text-slate-400 truncate">
                                      {JSON.stringify(originalEvent.tool_call?.arguments)}
                                    </div>
                                  </div>
                                  <div className="p-3 bg-pink-500/5 rounded-lg border border-pink-500/10">
                                    <div className="text-[9px] font-bold uppercase text-pink-500 mb-1">Simulated Payload</div>
                ...
                {data?.reasoning_audit && (
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-5 text-center">
                      <div className="text-[10px] font-bold text-slate-500 uppercase mb-2">Goal Alignment</div>
                      <div className="text-3xl font-bold text-blue-400">{(data.reasoning_audit.goal_alignment * 100).toFixed(0)}%</div>
                    </div>
                    <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-5 text-center">
                      <div className="text-[10px] font-bold text-slate-500 uppercase mb-2">Logic Consistency</div>
                      <div className="text-3xl font-bold text-purple-400">{(data.reasoning_audit.overall_score * 100).toFixed(0)}%</div>
                    </div>
                    <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-5 text-center">
                      <div className="text-[10px] font-bold text-slate-500 uppercase mb-2">Hallucination Risk</div>
                      <div className={`text-3xl font-bold ${data.reasoning_audit.hallucination_risk > 0.3 ? 'text-red-400' : 'text-green-400'}`}>
                        {(data.reasoning_audit.hallucination_risk * 100).toFixed(0)}%
                      </div>
                    </div>
                  </div>
                )}

                {!data?.failure_analysis && !data?.reasoning_audit?.findings.length ? (
                  <div className="flex flex-col items-center justify-center py-20 text-slate-500">
                    <CheckCircle2 size={48} className="mb-4 text-green-500/30" />
                  ...
                  <>
                    {data.failure_analysis && (
                      <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-8">
                        <div className="flex items-start gap-6">
                          ...
                          <div className="flex-1">
                            <div className="text-[10px] font-bold text-red-400 uppercase tracking-[0.2em] mb-2">Failure Report</div>
                            <h2 className="text-3xl font-bold text-white mb-2">{data.failure_analysis.primary_cause.replace(/_/g, ' ')}</h2>
                            <p className="text-slate-300 leading-relaxed text-lg">{data.failure_analysis.summary}</p>
                          </div>
                        </div>
                      </div>
                    )}

                    {data.reasoning_audit && data.reasoning_audit.findings.length > 0 && (
                      <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
                        <h3 className="text-sm font-bold text-slate-400 uppercase mb-4 flex items-center gap-2">
                          <MessageSquare size={16} className="text-purple-400" /> Reasoning Anomalies
                        </h3>
                        <div className="space-y-3">
                          {data.reasoning_audit.findings.map((finding: any, i: number) => (
                            <div
                              key={i}
                              className={`p-4 rounded-xl border flex gap-4 ${
                                finding.severity === 'high' ? 'bg-red-500/5 border-red-500/20' : 'bg-orange-500/5 border-orange-500/20'
                              }`}
                            >
                              <div className={`mt-1 h-2 w-2 rounded-full shrink-0 ${finding.severity === 'high' ? 'bg-red-500' : 'bg-orange-500'}`}></div>
                              <div>
                                <div className="text-[10px] font-bold uppercase text-slate-500 mb-1">
                                  Step #{finding.step_index + 1} — {finding.type}
                                </div>
                    ...
                    {data.failure_analysis && (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
                          <h3 className="text-sm font-bold text-slate-400 uppercase mb-4 flex items-center gap-2">
                            <Info size={16} className="text-blue-400" /> Contributing Factors
                          </h3>
                          <ul className="space-y-3">
                            {data.failure_analysis.contributing_factors.map((factor: string, i: number) => (
                        ...
                        <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
                          <h3 className="text-sm font-bold text-slate-400 uppercase mb-4 flex items-center gap-2">
                            <Zap size={16} className="text-yellow-400" /> Recommendations
                          </h3>
                          <ul className="space-y-3">
                            {data.failure_analysis.recommendations.map((rec: string, i: number) => (
                              <li key={i} className="flex items-start gap-3 text-sm text-slate-300">
                    ...
                    {data.failure_analysis?.tool_error_counts && Object.keys(data.failure_analysis.tool_error_counts).length > 0 && (
                      <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
                        ...
                        <div className="space-y-4">
                          {Object.entries(data.failure_analysis.tool_error_counts).map(([tool, count]: [string, any]) => (
                            <div key={tool}>
                              <div className="flex justify-between text-xs mb-1.5">
                                <span className="font-mono text-slate-300">{tool}</span>
                                <span className="text-red-400 font-bold">{count} failures</span>
                              </div>
                              <div className="w-full h-2 bg-slate-800 rounded-full overflow-hidden">
                                <div
                                  className="h-full bg-red-500 rounded-full"
                                  style={{ width: `${Math.min(100, (count / events.length) * 100 * 5)}%` }}
                                ></div>
                              </div>
                            </div>
                          ))}
          ...
          <div className="flex items-center justify-between px-6 py-1.5 bg-slate-900/50 border-b border-slate-800 text-[9px] font-bold text-slate-500 uppercase tracking-widest">
            <div className="flex gap-4">
              <span className="flex items-center gap-1.5">
                <div className="w-1.5 h-1.5 rounded-full bg-blue-500"></div> Tool Call
              </span>
              <span className="flex items-center gap-1.5">
              ...
              <span className="flex items-center gap-1.5">
                <div className="w-1.5 h-1.5 rounded-full bg-purple-500"></div> Reasoning
              </span>
            </div>
            <div>
                  ...
                  >
                    <div
                      className={`transition-all duration-300 rounded-full ${
                        isSelected ? 'w-4 h-4 ring-4 ring-pink-500/20' : 'w-2 h-2 hover:w-3 hover:h-3'
                      }`}
                      style={{
                        backgroundColor: isSelected ? '#ec4899' : EVENT_COLOR[ev.event_type] || '#475569',
