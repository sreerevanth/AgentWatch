...
from agentwatch.telemetry.otel import TelemetryProvider, TelemetryConfig, OTELConfig, OTELExporter

def test_telemetry_config_backward_compatibility():
    # Test that both new and legacy parameters work
    config = TelemetryConfig(
        endpoint="http://legacy:4317",
        insecure=True,
        headers={"x-test": "val"}
    )
...
def test_telemetry_flush_on_initialize():
    ...
    assert mock_exporter.export.called or True # If _OTEL_AVAILABLE is false, it uses fallback logger
