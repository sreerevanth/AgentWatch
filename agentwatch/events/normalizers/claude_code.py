...
class ClaudeCodeNormalizer(Normalizer):
    name = "claude_code"
    version = "1"
    source_kinds = frozenset({"claude_code.system", "claude_code.assistant", "claude_code.user", "claude_code.result", "claude_code.unparseable", "claude_code.summary"})
    ...
    def normalize(self, observations: Sequence[RawObservation], ctx: NormalizeContext) -> NormalizeResult:
        ...
        order: list[str] = []
        pending_results: dict[str, list[str]] = {}  # session -> tool_use ids answered since last assistant msg
        ...
        for obs in ordered:
            ...
            if t == "assistant":
                ...
                if not any(isinstance(b, dict) and b.get("type") == "tool_result" for b in (p.get("message") or {}).get("content") or []):
                ...
                result.diagnostics.append(Diagnostic(obs.obs_id, "warning", "unparsed_line", f"line type {t!r} not interpreted"))
        for mid in order:
            result.events.append(self._message(mid, messages[mid], depends.get(mid, []), session_of.get(mid), ctx))
        for tid in dict.fromkeys([*tool_use, *tool_result]):
            result.events.append(self._tool(tid, tool_use.get(tid), tool_result.get(tid), ctx, result))
        return result
    ...
    def _message(self, mid: str, obs: list[RawObservation], deps: list[str], sid: str | None, ctx: NormalizeContext) -> Any:
        ...
        b.resources = {k: v for k, v in {"tokens_in": usage.get("input_tokens"), "tokens_out": usage.get("output_tokens"),
                                          "cache_read_tokens": usage.get("cache_read_input_tokens")}.items() if v is not None}
        ...
        b.time = TemporalInfo(start=obs[0].observed_at, end=obs[-1].observed_at if len(obs) > 1 else None,
                              basis=obs[0].clock.source if obs[0].observed_at else "missing", ordering_key=f"{obs[0].sensor.instance_id}:{obs[0].source_seq}")
        return b.build()

    def _tool(self, tid: str, use: tuple[RawObservation, dict[str, Any], str | None] | None, res: tuple[RawObservation, dict[str, Any]] | None, ctx: NormalizeContext, result: NormalizeResult) -> Any:
        ...
        if use:
            ...
            b.operation = "unknown"
            result.diagnostics.append(Diagnostic(res[0].obs_id if res else None, "warning", "result_without_use", f"tool_result {tid} without tool_use"))
        if res:
            block = res[1]
    ...
    def _user_input(self, obs: RawObservation, p: dict[str, Any], ctx: NormalizeContext) -> Any:
        ...
        if obs.declared("parent_entry_uuid"):
            b.parents.append(DeclaredLink("follows", "claude_code.entry", obs.declared("parent_entry_uuid") or ""))
    ...
    def _lifecycle(self, obs: RawObservation, p: dict[str, Any], ctx: NormalizeContext) -> Any:
        ...
        if t == "result":
            ...
            b.resources = {k: v for k, v in {"cost_usd": p.get("total_cost_usd") or p.get("cost_usd"), "latency_ms": p.get("duration_ms"),
                                              "tokens_in": (p.get("usage") or {}).get("input_tokens"), "tokens_out": (p.get("usage") or {}).get("output_tokens")}.items() if v is not None}
            if p.get("result") is not None:
                b.output(p["result"], role="final_result")
            b.attributes = {"outcome": "error" if p.get("is_error") else "ok", "subtype": p.get("subtype"), "num_turns": p.get("num_turns")}
            if sid:
                b.source_ids = [("claude_code.result", sid)]
        else:
            b.operation = f"{t}:{p.get('subtype') or ''}".rstrip(":")
            b.status = EventStatus.OK
            b.attributes = {k: p.get(k) for k in ("model", "tools", "cwd", "permissionMode", "subtype") if p.get(k) is not None}
        b.time = temporal(obs)
        return b.build()
