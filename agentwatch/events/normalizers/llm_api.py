...
class LLMApiNormalizer(Normalizer):
    ...
    def normalize(self, observations: Sequence[RawObservation], ctx: NormalizeContext) -> NormalizeResult:
        ...
        for obs in source_order(observations):
            ...
            if not cid:
                result.diagnostics.append(Diagnostic(obs.obs_id, "error", "missing_call_id", "provider observation without call_id"))
                continue
    ...
    def _call(self, cid: str, parts: dict[str, RawObservation], ctx: NormalizeContext, result: NormalizeResult) -> Any:
        ...
        model = request.get("model") or (response.get("model") if isinstance(response, dict) else None)
        ...
        if end is None:
            ...
            result.diagnostics.append(Diagnostic(anchor.obs_id, "warning", "no_response", f"{self.provider} call {cid} has no response"))
...
class OpenAINormalizer(LLMApiNormalizer):
    ...
    def map_io(self, b: EventBuilder, request: dict[str, Any], response: Any) -> None:
        ...
        if request.get("tools"):
            b.attributes["tools_offered"] = [((t.get("function") or {}).get("name") or t.get("name")) for t in request["tools"] if isinstance(t, dict)]
        if isinstance(response, dict):
            ...
            b.resources = {k: v for k, v in {"tokens_in": usage.get("prompt_tokens"), "tokens_out": usage.get("completion_tokens")}.items() if v is not None}
...
class AnthropicNormalizer(LLMApiNormalizer):
    ...
    def map_io(self, b: EventBuilder, request: dict[str, Any], response: Any) -> None:
        prompt = {"system": request.get("system"), "messages": request.get("messages")} if request.get("system") else request.get("messages")
        if prompt is not None:
            b.input(prompt, role="prompt")
        if request.get("tools"):
            b.attributes["tools_offered"] = [t.get("name") for t in request["tools"] if isinstance(t, dict)]
        if isinstance(response, dict):
            texts = [c.get("text", "") for c in response.get("content") or [] if isinstance(c, dict) and c.get("type") == "text"]
            uses = [c for c in response.get("content") or [] if isinstance(c, dict) and c.get("type") == "tool_use"]
            if texts:
            ...
            if uses:
                ...
                b.facets.append("tool_call_request")
            usage = response.get("usage") or {}
            b.resources = {k: v for k, v in {"tokens_in": usage.get("input_tokens"), "tokens_out": usage.get("output_tokens")}.items() if v is not None}
