...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args:
        print("usage: python -m agentwatch.sensors.native.launch script.py [args...]", file=sys.stderr)
        return 2
    script = args[0]
    ...
    if not out:
        ...
        return 2
    rec = aw.configure(FileSink(out), system=os.environ.get("AGENTWATCH_SYSTEM") or Path(script).stem)
    ...
    with rec.run(Path(script).stem, run_id=os.environ.get("AGENTWATCH_RUN_ID"), command=command, cwd=os.getcwd(),
                 replay_of=os.environ.get("AGENTWATCH_REPLAY_OF"), branch_id=os.environ.get("AGENTWATCH_BRANCH_ID")) as run:
        try:
