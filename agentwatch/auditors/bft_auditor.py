...
class BFTAuditor:
    ...
    def __init__(
        ...
        self.consensus_engine = ConsensusEngine(
            threshold=consensus_threshold, trust_scorer=self.trust_scorer
        )
    ...
    async def audit_step(
        ...
        for auditor in self.auditors:
            task = asyncio.create_task(
                self._audit_with_timeout(auditor, step_number, step_data)
            )
            tasks.append(task)
        ...
        results = []
        try:
            # Use wait instead of wait_for to handle partial results
            ...
            logger.error(f"Error during auditor execution: {e}")

        # Filter out errors and get valid results
        valid_results = [r for r in results if isinstance(r, AuditResult)]

        if len(valid_results) < self.quorum_size:
            logger.warning(
                f"Quorum not reached: {len(valid_results)} < {self.quorum_size}"
            )
