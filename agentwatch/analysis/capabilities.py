...
def _define() -> None:
    E, V, P = Maturity.EXPERIMENTAL, Maturity.VALIDATED, Maturity.PRODUCTION
    items = [
        ...
        ("evidence.edge_redaction", "1", V, "secret redaction before storage with manifests; inputs never mutated", ("tests/v3/test_evidence.py",)),
        ("events.normalization.native", "1", V, "native SDK spans → computational events", ("tests/v3/test_normalizers.py",)),
        ("events.normalization.legacy", "1", V, "v0.2 AgentEvent translation with loss accounting", ("tests/v3/test_normalizers.py",)),
        ("events.normalization.otel", "1", E, "OTel/GenAI spans → events", ()),
        ("events.normalization.langchain", "1", E, "LangChain callbacks → events (parent_run_id preserved)", ()),
        ("events.normalization.claude_code", "1", E, "Claude Code stream-json/transcripts → events", ()),
        ("events.normalization.provider_sdks", "1", E, "OpenAI/Anthropic client wrappers, MCP tap", ()),
        ...
        ("causality.hypotheses", "1", E, "hypotheses with computed evidence classes; causes/effects", ()),
        ("lab.replay", "1", E, "replay L0-L3 with reproduction confidence", ()),
        ("lab.branch", "1", E, "branching timelines with substitution", ()),
        ("lab.counterfactual", "1", E, "counterfactual framework (SIMULATED/MODEL_ESTIMATED/UNKNOWN)", ()),
        ("state.latent", "1", E, "interpretable latent-state baseline", ()),
        ("forecasting.trajectory", "1", E, "kNN trajectory forecasts with calibration evaluation", ()),
