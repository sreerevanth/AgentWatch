...
def create_branch(ws: Workspace, run_ref: str, at_event: str, substitution: Any, *, level: str = "L2", note: str | None = None) -> dict[str, Any]:
    ...
    if not key:
        raise ValueError("branching requires an instrumented call (event has no call_key); use aw.tool/aw.model/aw.retriever")
    ...
    doc = {"source_run": run["run_id"], "branch_event": ev["event_id"], "call_key": key, "substitution": substitution,
           "substitution_preview": canonical_json(substitution)[:200], "replay_level": level, "note": note,
           "parent_branch": parent_branch, "event_label": f"{ev['kind']} {ev['operation']}"}
    bid = ws.store.put_experiment(ws.tenant_id, "branch", doc, subject=run["run_id"])
    return {**doc, "branch_id": bid}


def execute_branch(ws: Workspace, branch_id: str, *, live: list[str] | None = None) -> dict[str, Any]:
    br = ws.store.experiment(branch_id)
    if br is None or br.get("record_type") != "branch":
        raise LookupError(f"branch {branch_id} not found")
    result = replay(ws, br["source_run"], br["replay_level"], substitutions={br["call_key"]: br["substitution"]}, live=live, branch_id=branch_id)
    ...
    if branch_run:
        ...
        final_changed = _final_outputs(fresh, source["run_id"]) != _final_outputs(fresh, branch_run["run_id"])
    doc = {
        "branch_id": branch_id,
        ...
        "label": "SIMULATED",
        "outcome": {"source": source.get("status"), "branch": branch_run.get("status") if branch_run else None},
...
def _final_outputs(ws: Workspace, run_id: str) -> list[str]:
    return sorted(a["artifact_id"] for e in ws.events(run_id) if "artifact_creation" in e["facets"] for a in e["outputs"])
...
def branch_history(ws: Workspace, run_ref: str) -> dict[str, Any]:
    ...
    results = {r["branch_id"]: r for r in ws.store.experiments(ws.tenant_id, "branch_result", subject=run["run_id"])}
    return {"run_id": run["run_id"], "branches": [{**b, "branch_id": b["record_id"], "result": results.get(b["record_id"])} for b in branches]}


def counterfactual(ws: Workspace, run_ref: str, at_event: str, alternative: Any, *, execute: bool = True, level: str = "L3") -> dict[str, Any]:
    ...
    base = {
        ...
        "event": ev["event_id"],
        "observed": {"label": "OBSERVED", "run_status": run.get("status"), "event_status": ev["status"]},
        "maturity": "EXPERIMENTAL",
    }
    if execute and ev["attributes"].get("call_key") and (run.get("attributes") or {}).get("command"):
        try:
            br = create_branch(ws, run["run_id"], ev["event_id"], alternative, level=level, note="counterfactual")
            res = execute_branch(ws, br["branch_id"])
            return {**base, "method": f"re-execution ({level})", "estimate": {
                "label": "SIMULATED",
                ...
                "uncertainty": {"reproduction_confidence": res["reproduction_confidence"],
                                "live_components": res["live_components"], "missing_dependencies": res["missing_dependencies"],
                                "note": "single re-execution; live components (program logic, clocks, networks) may vary between executions"},
            }, "branch_id": br["branch_id"], "experiment_id": res["record_id"]}
...
def _estimate_from_history(ws: Workspace, run: dict[str, Any], ev: dict[str, Any], alternative: Any) -> dict[str, Any]:
    ...
    n = len(matches)
    if n < 3:
        return {"method": "historical comparison", "estimate": {"label": "UNKNOWN", "reason": f"only {n} recorded runs where this operation produced the alternative (need ≥3)", "supporting_runs": [m["run_id"] for m in matches]}}
    errors = sum(1 for m in matches if m.get("status") == "ERROR")
    return {"method": "historical comparison (runs of the same program where the alternative actually occurred)", "estimate": {
        "label": "MODEL_ESTIMATED",
        "outcome_distribution": {"ERROR": round(errors / n, 4), "not ERROR": round(1 - errors / n, 4)},
