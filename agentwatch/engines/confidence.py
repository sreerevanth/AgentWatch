...
def _check_tool_loop(events: List[AgentEvent]) -> Tuple[float, List[str]]:
    ...
    # Sliding window: look for repeated N-grams
    for window in (3, 2):
    ...
    # High repetition of single tool
...
def _check_hallucinated_success(events: List[AgentEvent]) -> Tuple[float, List[str]]:
    ...
    # Check for blocked actions or errors in the run
...
def _check_goal_alignment(
    ...
    # Gather observable text from execution
    ...
    if not observable_texts:
        return 0.8, []  # No observable data — uncertain
    ...
    score = min(1.0, overlap * 2)  # Scale up since partial overlap is normal
...
def _check_context_explosion(events: List[AgentEvent]) -> Tuple[float, List[str]]:
    ...
    # Check if last reading is > 5x the average of first few
...
class ConfidenceScorer:
    ...
    def score(
        ...
        # Consistency = average of non-goal components
